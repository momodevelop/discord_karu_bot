...
class cResponse extends cResponseBase {
	...
	public async exec(params: cCallbackParams): Promise<boolean> {
		...
		if (!f(c, ["regular", "turf"]) && f(c, ["all", "schedule"])) {
			return false;
		}
		...
		for (let i: number = 0; i < r.regular.length; ++i) {
			const result: iRegularScheduleInfo = r.regular[i];
			...
			const name = sprintf("%s", date.toLocaleString());
